const Coinflip = require('mongoose').model('CoinflipDARKCRYSTAL');
    ...
    addHistory: async (req, res) => {
        if (req.get('origin') === process.env.CLIENT_ORIGIN) {
            try {
                ...
                if (findRes) {
                    await Wallet.findOneAndUpdate({ account }, { totalBetDARKCRYSTAL: findRes.totalBetDARKCRYSTAL + amount });
                }
                req.app.get('io').emit('COINFLIP_DARKCRYSTAL', createRes);
        ...
        if (req.get('origin') === process.env.CLIENT_ORIGIN) {
            try {
                ...
                if (allRes) {
                    ...
                    res.status(200).json({
                        totalBetAmountDARKCRYSTAL: sumAll,
                        totalBetCountDARKCRYSTAL: allRes.length,
                    });
                } else {
                    res.status(200).json({
                        totalBetAmountDARKCRYSTAL: 0,
                        totalBetCountDARKCRYSTAL: 0,
                    });
                }
            } catch (err) {
