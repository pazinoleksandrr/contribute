const Coinflip = require('mongoose').model('CoinflipCRO');
    ...
    addHistory: async (req, res) => {
        if (req.get('origin') === process.env.CLIENT_ORIGIN) {
            try {
                ...
                if (findRes) {
                    await Wallet.findOneAndUpdate({ account }, { totalBet: findRes.totalBet + amount });
                }
                req.app.get('io').emit('COINFLIP_CRO', createRes);
    ...
    getStatsData: async (req, res) => {
        if (req.get('origin') === process.env.CLIENT_ORIGIN) {
            try {
                ...
                res.status(200).json({
                    totalBetAmount: sumAll,
                    totalBetCount: allRes.length,
                });
            } catch (err) {
