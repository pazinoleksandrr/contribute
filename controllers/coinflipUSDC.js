const Coinflip = require('mongoose').model('CoinflipUSDC');
    ...
    addHistory: async (req, res) => {
        if (req.get('origin') === process.env.CLIENT_ORIGIN) {
            try {
                ...
                if (findRes) {
                    await Wallet.findOneAndUpdate({ account }, { totalBetUSDC: findRes.totalBetUSDC + amount });
                }
                req.app.get('io').emit('COINFLIP_USDC', createRes);
        ...
        if (req.get('origin') === process.env.CLIENT_ORIGIN) {
            try {
                ...
                if (allRes) {
                    ...
                    res.status(200).json({
                        totalBetAmountUSDC: sumAll,
                        totalBetCountUSDC: allRes.length,
                    });
                } else {
                    res.status(200).json({
