const Coinflip = require('mongoose').model('CoinflipMNG');
    ...
    addHistory: async (req, res) => {
        if (req.get('origin') === process.env.CLIENT_ORIGIN) {
            try {
                ...
                if (findRes) {
                    await Wallet.findOneAndUpdate({ account }, { totalBetMNG: findRes.totalBetMNG + amount });
                }
                req.app.get('io').emit('COINFLIP_MNG', createRes);
        ...
        if (req.get('origin') === process.env.CLIENT_ORIGIN) {
            try {
                ...
                if (allRes) {
                    ...
                    res.status(200).json({
                        totalBetAmountMNG: sumAll,
                        totalBetCountMNG: allRes.length,
                    });
                } else {
                    res.status(200).json({
                        totalBetAmountMNG: 0,
