const Coinflip = require('mongoose').model('CoinflipMMF');
    ...
    addHistory: async (req, res) => {
        if (req.get('origin') === process.env.CLIENT_ORIGIN) {
            try {
                ...
                if (findRes) {
                    await Wallet.findOneAndUpdate({ account }, { totalBetMMF: findRes.totalBetMMF + amount });
                }
                req.app.get('io').emit('COINFLIP_MMF', createRes);
        ...
        if (req.get('origin') === process.env.CLIENT_ORIGIN) {
            try {
                ...
                if (allRes) {
                    ...
                    res.status(200).json({
                        totalBetAmountMMF: sumAll,
                        totalBetCountMMF: allRes.length,
                    });
                } else {
                    res.status(200).json({
                        totalBetAmountMMF: 0,
